...
> derives byte-identical tokens, and both implementations are held to one set
> of canonical test vectors by a sibling repository,
> [TOTPConformanceTests](https://github.com/OpenChargingCloud/TOTPConformanceTests)
> — the written specification, the vectors (generated by an independent third
> implementation and vendored here under `test/vectors/`), and a harness per
> implementation. The format was developed for **OCPP v2.1**, which specifies
...
`test/vectors/` (`test/vectors.test.ts`) — regenerate them in the
[TOTPConformanceTests](https://github.com/OpenChargingCloud/TOTPConformanceTests)
repository, never edit them here.
